...
function ProductHeartIcon({ size = 14, filled }) {
  return (
    <span className={`relative inline-flex items-center justify-center transition-all duration-300 ${filled ? "scale-[1.4]" : "scale-100"}`}>
      ...
      {filled && (
        <span
          className="absolute text-white font-bold leading-none pointer-events-none"
...
export default function ProductCard({ product, contextQueue }) {
  ...
  const typeBadge = (() => {
    if (product.type === "merch") {
      ...
      const category = CATEGORIES.find((item) => item.key === merchType);
      return {
        icon: MERCH_TYPE_ICONS[merchType] || "\u25C8",
        label: category?.label || "Merch",
      };
    }
            ...
            <>
              ...
              <button
                ...
                className={`relative flex items-center justify-center px-3 py-2.5 transition-all duration-200 ${
                  wishlisted ? "scale-110 text-accent" : "text-white/70 hover:scale-110 hover:text-accent"
                }`}
          ...
          <div className="mt-1 flex flex-wrap gap-1">
            {genres.slice(0, 2).map((genre) => (
              <span
                ...
                className="rounded bg-white/[0.07] px-1.5 py-0.5 text-[10px] leading-none text-white/50"
              >
                {genre.name}
              </span>
            ))}
