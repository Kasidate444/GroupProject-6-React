  ...
  return (
    <div className="min-h-screen bg-bg font-['Plus_Jakarta_Sans',sans-serif]">
      <div className="px-[5%] pb-6 pt-10">
        ...
        <p className="mt-1.5 text-[16px] text-white/50">
      ...
      <div className="space-y-4 px-[5%] pb-6">
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <FilterPill active={selectedGenres.length === 0} onClick={() => navigate("/shop")}>
              all genres
            </FilterPill>
            {genres.map((genre) => (
              <FilterPill
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {hasActiveFilters && (
              <button
                onClick={clearAll}
                className="ml-2 text-[11px] text-white/30 underline underline-offset-2 transition-colors hover:text-white/60"
              >
                clear all
              </button>
            )}
      ...
      <div className="px-[5%] py-6">
        ...
        {loadError ? (
          <div className="py-24 text-center">
            <p className="text-[15px] text-[#fc3c44]">{loadError}</p>
          </div>
        ) : loading ? (
          <ShopSkeletonGrid />
        ) : filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-x-3 gap-y-5 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-7">
            {filtered.map((product) => (
              <ProductCard key={product._id} product={product} contextQueue={filtered} />
            ))}
          </div>
        )}
      </div>
...
function ShopSkeletonGrid() {
  return (
    <div className="grid grid-cols-2 gap-x-3 gap-y-5 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-7">
      {Array.from({ length: SHOP_SKELETON_COUNT }).map((_, index) => (
...
function FilterPill({ active, onClick, children }) {
  return (
    <button
      onClick={onClick}
      className={`rounded-full px-4.5 py-1.75 text-[15px] font-medium transition-all ${
        active
...
function CategoryPill({ active, icon, onClick, children }) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-1.5 rounded-full border px-3.75 py-1.75 text-[14px] font-medium transition-all ${
        active
          ? "border-[rgba(252,60,68,0.45)] bg-[rgba(252,60,68,0.15)] text-white"
