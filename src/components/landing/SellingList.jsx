...
function ItemRow({ items, hidden = false }) {
  return (
    <div className="flex shrink-0 animate-[marquee_30s_linear_infinite] flex-row items-center gap-4 pr-4" aria-hidden={hidden}>
      {items.map((item, index) => (
        ...
        >
          <div className="relative overflow-hidden">
            <img className="aspect-square w-full object-cover transition-transform duration-500 group-hover:scale-105" src={item.cover_url} alt={hidden ? "" : item.title} />
  ...
  return (
    <div className="flex shrink-0 animate-[marquee_30s_linear_infinite] flex-row items-center gap-4 pr-4" aria-hidden={hidden}>
      {Array.from({ length: 5 }).map((_, index) => (
        <div key={`${hidden}-${index}`} className="flex w-40 shrink-0 flex-col overflow-hidden rounded-2xl border border-white/8 bg-[#141420]">
  ...
  return (
    <section>
      <div className="flex items-baseline gap-4 bg-[#03030f] px-[5%] pb-4 pt-8 after:h-px after:flex-1 after:bg-linear-to-r after:from-white/15 after:to-transparent after:content-[''] md:px-[10%]">
        <h2 className="font-['Plus_Jakarta_Sans',sans-serif] text-[20px] font-extrabold uppercase text-white md:text-[28px]">Selling right now</h2>
      </div>
